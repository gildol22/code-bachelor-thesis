%=========================================================================
% function [u,his] = ROFdecom_FPGS(f,lambda,varargin)
%
...
stopTol      = 1e-8;            % tolerance for stopping criteria
...
displayROF   = true;            % print step infos of procedure?
...
lh2 = 2*lambda*prod(h.^2);   % computational constants
...
TVnorm = @(a) sum(sqrt( ((a(3:end,2:end-1) - a(2:end-1,2:end-1))/h(1)).^2 +...
    ...
    % TV = sqrt(DplusX^2 + DplusY^2); a incl. border lines

    
%define J-functional (to be minimised here)
J = @(u) TVnorm(u) + lambda*L2norm(f-u(2:end-1,2:end-1));
    %Note: dimension of u includes artificially added border lines
