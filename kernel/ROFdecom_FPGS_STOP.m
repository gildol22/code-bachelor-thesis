%=========================================================================
% function [u,his] = ROFdecom_FPGS_STOP(f,lambda,varargin)
%
...
% - advanced output to compare more quantities (e.g. stopping strategies)
%
...
%                 [iter, J, Jold-J, (Jold-J)/Jold, |u|_BV, |f-u|, ...
%                  |uOld-u|, |uOld-u|/|uOld|, <u,f-u>, |u|_BV/2lambda, DIFF]
%=========================================================================
...
stopTol      = 1e-7;            % tolerance for stopping criteria
...
dualPair = @(a,b) sum(a.*b,'all') * prod(h); %dual pair; a,b interior
...
% advanced history and output initialisation
historyROF = displayROF | (nargout>1); % save infos on procedure history?

if(historyROF)
  his       = zeros(maxIter+1,11);
  Jold = J(uc);
  his(1,:)  = [0,Jold,0,0,TVnorm(uc),L2norm(f-u0),L2norm(u0),0,...
    dualPair(u0,f-u0),TVnorm(uc)/(2*lambda),0];
  
  if(displayROF)
    hisStr    = {'iter','J','Jold-J','(Jo-J)/Jo','|u|_BV','|f-u|',...
      '|uOld-u|','|uo-u|/|uo|','<u,f-u>','|u|_BV/2lam','DIFF'};
    
    % first output
    fprintf('%4s %-12s %-11s %-11s %-11s %-11s %-11s %-11s %-11s %-11s %4s\n%s\n',...
      hisStr{:},char(ones(1,100)*'-'));
    dispHis = @(var) ...
      fprintf('%4d %-12.4e %-11.3e %-11.3e %-11.3e %-11.3e %-11.3e %-11.3e %-11.3e %-11.3e %-11.3e\n',var);
    dispHis(his(1,:));
  end
end
...
for iter=1:maxIter              % number of iteration
  ...
  if(historyROF)
    Jc = J(uc);
    uTV = TVnorm(uc);
    ufuDual = dualPair(uc(2:end-1,2:end-1), f-uc(2:end-1,2:end-1));
    his(iter+1,:) = [iter, Jc, Jold-Jc, (Jold-Jc)/Jold, uTV, ...
                     L2norm(f-uc(2:end-1,2:end-1)), uDiff, ...
                     uDiff/L2norm(uOld(2:end-1,2:end-1)), ...
                     ufuDual, uTV/(2*lambda), abs(ufuDual-uTV/(2*lambda))];
  ...
  if(visualiseROF)
    pause(pauseTime)
    im_uc.CData = uc(2:end-1,2:end-1)';
