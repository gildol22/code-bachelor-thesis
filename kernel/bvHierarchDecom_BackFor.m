%=========================================================================
% function [uHis,vm,his,his0,hisROF] = bvHierarchDecom_BackFor(f,lambda0,varargin)
%
% computes hierarchical (BV,L2) image decomposition with backward
% initialisation (m0 times) and forward procedure with m+1 instances
%
...
%   his         [matrix] optional information on fw. procedure history with:
%                        [l, lambda, |Sum(u)|_BV, |u|_BV, |v|_2, iterROF]
%   his0        [matrix] optional information on first decomposition with:
%                        [lambda, |hat(u)|_BV, |v|_2, iterROF]
%   hisB        [matrix] optional information on bw. procedure history with:
%                        [l, lambda, |hat(u)|_BV, |u|_BV, |v|_2, iterROF]
%   hisROF      [matrix] optional infos on each decomp.'s history with:
%                        [iter, J, Jold-J, |u|_BV, |f-u|, |uOld-u|]
%=========================================================================

function [uHis,vm,his,his0,hisB,hisROF] = bvHierarchDecom_BackFor(f,lambda0,varargin)
...
m            = 10;              % number of forward decomp. instances (-1)
m0           = 5;               % number of backward decomp. instances
...
stopTolROF   = 3e-8;            % tolerance for stopping criteria
...
% -- initialise  ---------------------------------------------------------
uHis = zeros([size(f), m]); % history of BV parts from fw. ROF decomp.'s
uHisB= zeros([size(f), m0+1]); % history of residuals from bw. ROF decomp.'s
hisROF = zeros(maxIterROF+1,6,m); % history output of fw. ROF decomp.'s
hisROFB= zeros(maxIterROF+1,6,m0);  % history output of bw. ROF decomp.'s
hisROF0= zeros(maxIterROF+1,6);     % history output of first ROF decomp.
...
ROFinitBackw  = 'alpha*uHATc + (1-alpha)*mean(uHATc,''all'')';
...
[uHATc, hisROF0(:,:)] = eval([ROFdecomMeth, '(f, lambda0, ', ...
...
vc = f-uHATc;             % current residual v_l, here v_0
...
if(historyHie)
  iterROF   = max(hisROF0(:,1));
  his0 = [lambda0, hisROF0(iterROF+1,4), hisROF0(iterROF+1,5), iterROF];
  
  if(displayHie)
    hisStr    = {'','lambda','|hat(u0)|_BV','','|v0|_2','iterROF'};
    
    % first output
    fprintf('--FIRST--\n%3s %-10s %-12s %-11s %-11s %4s\n%s\n',...
      hisStr{:},char(ones(1,60)*'-'));
    dispHis = @(var) ...
      fprintf('%3d %-10.2e %-12.4e %-11.3e %-11.3e %3d\n',var);
    dispHis([0, his0(1:2), 0, his0(3:4)]);
  end
end
...
if(visualiseHie)
  ...
  subplot(2,2,2)   %Sum(u)
  [im_uSum,ax_uSum] = im2Dsetup(uHATc,D,colourLim);
  ax_uSum.Title = title('$\hat{u}_0$', ...
                        'interpreter', 'latex');
  
  subplot(2,2,3)   %uc
  [im_uc,ax_uc] = im2Dsetup(uHATc,D,colourLim);
  ax_uc.Title = title('$\hat{u}_0$', ...
                        'interpreter', 'latex');
...
if(historyHie)
  ...
  if(displayHie)
    hisStr    = {'l','lambda','|hat(u)|_BV','|u|_BV','|u|_2','iterROF'};
...
backwBreak = false;        %break in backwards loop?

for l=0:-1:-m0+1
  
  %If optimal initialisation chosen, compute start value u0
  if(strcmp(ROFinitMeth,'optimal'))
    alpha = ROFinitOptimal(uHATc,lambda0*s^l,'D',D);
  end
  
  
  %Compute next decomposition and residual
  uHATold = uHATc;
  [uHATc, hisROFB(:,:,1-l)] = eval([ROFdecomMeth, ...
    '( uHATold, lambda0*s^(l-1), ''D'', D, ''u0'', ', ROFinitBackw, ...
    ', ''maxIter'', maxIterROF, ''eps2'', eps2ROF, ', ...
    '''stopTol'', stopTolROF, ''displayROF'', displayROF, ', ...
    '''visualiseROF'', visualiseROF, ''pauseTime'', pauseTimeROF, ', ...
    '''colourLim'', colourLim )']);
  
  uHisB(:,:,l+end) = uHATold - uHATc;
  
  %history and output of latest step
  if(historyHie)
    iterROF = max(hisROFB(:,1,1-l));
    hisB(1-l,:) = [l, lambda0*s^(l-1), hisROFB(iterROF+1,4,1-l), ...
              TVnorm(uHisB(:,:,l+end)), hisROFB(iterROF+1,5,1-l), iterROF];
               %{'l','lambda','|hat(u)|_BV','|u|_BV','|u|_2','iterROF'}
    if(displayHie), dispHis(hisB(1-l,:)); end
  end
  ...
  if(visualiseHie)
    ...
    subplot(2,2,3)   %uHATc
    [im_uc,ax_uc] = im2Dsetup(uHATc,D,colourLim);
    ax_uc.Title   = title(['$\hat{u}_{', num2str(l-1), '}$'], ...
                        'interpreter', 'latex');

    subplot(2,2,4)   %uc
    [im_vc,ax_vc] = im2Dsetup(uHisB(:,:,l+end),D,colourLimDiff);
    ax_vc.Title   = title(['u_{', num2str(l), '}+0.5']);
  end
  
end

% TODO: implementation of optional break in backward initialisation


if(~backwBreak), uHisB(:,:,1) = uHATc; end
...
for l=1:m
  ...
  %Compute next decomposition and residual
  [uHis(:,:,l), hisROF(:,:,l)] = eval([ROFdecomMeth, ...
    '( vc, lambda0*s^l, ''D'', D, ''u0'', ', ROFinitFollow, ...
    ', ''maxIter'', maxIterROF, ''eps2'', eps2ROF, ', ...
    '''stopTol'', stopTolROF, ''displayROF'', displayROF, ', ...
    '''visualiseROF'', visualiseROF, ''pauseTimeROF'', pauseTimeROF, ', ...
  ...
  %history and output of latest step
  if(historyHie)
    iterROF = max(hisROF(:,1,l));
    his(l,:) = [l, lambda0*s^l, TVnorm(sum(uHis(:,:,:),3)), ...
                  hisROF(iterROF+1,4,l), hisROF(iterROF+1,5,l), iterROF];
                 %{'l','lambda','|Sum(u)|_BV','|u|_BV','|v|_2','iterROF'}
    if(displayHie), dispHis(his(l,:)); end
  end
  ...
  if(visualiseHie)
    ...
    subplot(2,2,2)   %Sum(u)
    [im_uSum,ax_uSum] = im2Dsetup(sum(uHisB(:,:,:),3)+sum(uHis(:,:,:),3),D,colourLim);
    ax_uSum.Title = title(['$\Sigma_{\ell=-', num2str(m0),'}^{', ...
                           num2str(l),'} u_\ell$'],'interpreter', 'latex');

    subplot(2,2,3)   %uc
    [im_uc,ax_uc] = im2Dsetup(uHis(:,:,l),D,colourLimDiff);
...
uHis = cat(3,uHisB,uHis);               %final decomposition
vm=vc;                                  %final residual
hisROF = cat(3,hisROFB,hisROF0,hisROF); %all ROF histories combined
